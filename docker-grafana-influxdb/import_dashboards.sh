...
#
############################
# shamelessly stolen from http://gist.github.com/thedoc31/628beeee934f9c84648c108d4ad89f05
############################
...
for DASH_FILE in $DASH_LIST; do 
    ...
    RESULT=$(cat "$DASH_FILE" | jq '. * {overwrite: true, dashboard: {id: null}}' | curl -s -X POST -H "Content-Type: application/json" -H "Authorization: Bearer $KEY" "http://$TARGET_HOST/api/dashboards/db" -d @-)
